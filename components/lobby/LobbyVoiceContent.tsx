...
import { User, Gift, Users, ArrowRight, ArrowLeft } from "lucide-react";
...
export default function LobbyVoiceContent({ host, cohosts, speakingUser }: LobbyVoiceContentProps) {
  ...
  const [stageSpeakers] = useState([
    ...
    ...cohosts.filter(c => c.online).map(c => ({ id: c.id, name: c.name, avatar: c.avatar, role: 'Cohost' }))
  ]);
  ...
  return (
    <div className={`relative w-full aspect-video bg-white/80 dark:bg-gray-800/80 border-b border-gray-200 dark:border-gray-700 rounded-b-lg shadow-md overflow-hidden transition-all duration-300 ${charityMinimized ? 'flex flex-col items-center justify-center' : 'flex flex-row items-stretch justify-center'}`}>
      {/* Host & Cohosts Area */}
